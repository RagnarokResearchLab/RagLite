...
local function normalizePathSeparators(cstr, len)
    ...
    for readIndex = 0, len - 1 do
        ...
        if c == string_byte("\\") then
            cstr[writeIndex] = string_byte("/")
        elseif c ~= string_byte("/") or writeIndex ~= 0 then  -- Skip leading slashes
            cstr[writeIndex] = c
        else
            -- Intentionally leave writeIndex unchanged, effectively skipping this character
        end

        if writeIndex > 0 or c ~= string_byte("/") then  -- Don't advance writeIndex for leading '/'
