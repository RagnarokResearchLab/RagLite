...
describe("RagnarokRSW", function()
	describe("DecodeFileContents", function()
		it("should be able to decode RSW files using version 1.9 of the format", function()
			...
			-- This currently differs from the GND structs - streamline later
			assertEquals(rsw.waterPlane.normalizedSeaLevel, -100)
			assertEquals(rsw.waterPlane.textureTypePrefix, 0)
			assertEquals(rsw.waterPlane.waveformAmplitude, 1)
			assertEquals(rsw.waterPlane.waveformPhaseLength, 2)
			assertEquals(rsw.waterPlane.surfaceCurvatureOffset, 37)
			assertEquals(rsw.waterPlane.numTextureCyclingIntervals, 4)
			assertEqualNumbers(rsw.waterPlane.normalizedCycleTimeInMilliseconds, 2133.33, 1E-1)
